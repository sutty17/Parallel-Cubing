...
FILE *outFile;
#pragma omp parallel num_threads(12) shared(cu,optimalDist,nodes,tests) private(outFile)
{
...
#pragma omp barrier
char outFileName[50];
sprintf(outFileName,"OpenMPMovesAllowedThread%dOut.txt",ID);
...
if(first){fprintf(outFile,"First move: %s\n",mv[snP->move]);first=0;}
//fprintf(outFile,"%s\n",mv[snP->move]);
//fflush(outFile);
while(snP->move ==-1){
	if (r_depth==manLength)
	{
		...
		r_depth +=2;
		manLength = r_depth;
		if (manLength>optimalDist+2*subOptLev) ret = 1;
