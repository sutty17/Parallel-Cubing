...
gettimeofday(&start,NULL);
  while (count != NGOAL)
    {
#pragma omp parallel for private(flipSlice, twist1, twist, parity, idx1, idx, parity1, symFlipSlice1, sym1, flipSlice1, m) shared (visitedA, visitedB, twistMove, twistConjugate, count)
	for(flipSlice = 0; flipSlice < NFLIPSLICE; flipSlice++)
      {
	  for (twist = 0; twist < NTWIST; twist++)
		...
		if (visitedA[idx >> 3] & 1 << (idx & 7))
		...
		else
		  {
			...
			idx1 =
			  ((NTWIST * flipSlice1 + twist1) << 1) + parity1;
			#pragma omp critical
			{
...
printf("Time to generate table: %lf\n",1.0*msecs/1000000.0);
...
MOVE:
  ...
  if (snP->move == -1)		//no more possible move, go to left node
	...
	{
	  printf ("depth %2u completed, %14" PRIu64 " nodes", manLength,
		  nodes);
...
//which is lexically earlier. Ignore those moves.
  if (symRed == 1 && (sym1 & GESymmetries[m]) != sym1)
    goto MOVE;

  if (manLength - r_depth >= 2)	//Prevent UUU, RRR, FFF...
    if (m == (snP - 1)->move && m == (snP - 2)->move)
      goto MOVE;
...
//to the target group.
  ...
  snPNew->movesCloserTargetR =
    moveBitsConjugate[movesCloserToTarget[twistConjR]
		      [(snPNew->flipSliceR << 1) +
		       snPNew->parity]][symIdxMultiply[snPNew->symR][16]];
  snPNew->movesCloserTargetF =
    moveBitsConjugate[movesCloserToTarget[twistConjF]
		      [(snPNew->flipSliceF << 1) +
		       snPNew->parity]][symIdxMultiply[snPNew->symF][32]];
