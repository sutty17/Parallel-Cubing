...
sprintf(groupOutFileName,"OpenMPVariableDepthGroup%dout.txt",ID);
...
allOut = fopen("OpenMPVariableDepthAllOut.txt","w");
FILE *tempIn = NULL;
char inFileName[50];
char line[100];
int threadCounter;
...
for(threadCounter=0;threadCounter<NUM_GROUPS;threadCounter++){
	sprintf(inFileName,"OpenMPVariableDepthGroup%dout.txt",threadCounter);
	tempIn = fopen(inFileName,"r");
	while(fgets(line,sizeof line,tempIn) != NULL){
		fprintf(allOut,line);
		fflush(allOut);
