...
sprintf(groupOutFileName,"OpenMPHybridVariableDepthGroup%dout.txt",ID);
...
allOut = fopen("OpenMPHybridVariableDepthAllOut.txt","w");
FILE *tempIn = NULL;
char inFileName[50];
char line[100];
int threadCounter;
...
for(threadCounter=0;threadCounter<NUM_GROUPS;threadCounter++){
	sprintf(inFileName,"OpenMPHybridVariableDepthGroup%dout.txt",threadCounter);
	tempIn = fopen(inFileName,"r");
	while(fgets(line,sizeof line,tempIn) != NULL){
		fprintf(allOut,line);
		fflush(allOut);
